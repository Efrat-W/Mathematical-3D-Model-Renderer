...
class MiniProject2Tests {
	...
	void MiniProject2Test() {
		...
		while (tmpy < y1) {
			...
			g.add(new Polygon(new Point(299, tmpy, z1), new Point(299, tmpy + margin, z1),
					new Point(299, tmpy + margin, z2), new Point(299, tmpy, z2)).setEmission(new Color(BLACK))
		...
		while (tmpz < z2) {
			...
			g.add(new Polygon(new Point(299, y2, tmpz), new Point(299, y2, tmpz + margin),
					new Point(299, y1, tmpz + margin), new Point(299, y1, tmpz)).setEmission(new Color(BLACK))
					.setMaterial(material1));
			tmpz += win_w + margin;
		}
		...
		while (tmpy < y1) {
			...
			g.add(new Polygon(new Point(x + margin, tmpy, z1), new Point(x + margin, tmpy + margin, z1),
					new Point(x + margin, tmpy + margin, z2), new Point(x + margin, tmpy, z2))
					.setEmission(new Color(GRAY)).setMaterial(material1));
			tmpy += win_h + margin;
		}
		...
		Geometries building2 = new Geometries(
				new Polygon(p1, p2, p4, p3).setMaterial(material2).setEmission(new Color(BLACK))//
		...
		while (tmpz < z2) {
			...
			g.add(new Polygon(new Point(x, y2 - 2, tmpz), new Point(x, y2 - 2, tmpz + margin),
					new Point(-1060, y2 - 2, tmpz + margin), new Point(-1060, y2 - 2, tmpz))
					.setEmission(new Color(GRAY)).setMaterial(material1));
			tmpz += win_w + margin;
		}
		x = 400;
		...
		Geometries building3 = new Geometries( //
		...
		while (tmpx < 0) {
			...
			g.add(new Polygon(new Point(tmpx, y2(tmpx), z1), new Point(tmpx - margin, y2(tmpx - margin), z1),
					new Point(tmpx - margin, y2(tmpx - margin), z2), new Point(tmpx, y2(tmpx), z2))
		...
		while (tmpx > -270) {
			...
			g.add(new Polygon(new Point(tmpx, y3(tmpx), z1), new Point(tmpx - margin, y3(tmpx - margin), z1),
					new Point(tmpx - margin, y3(tmpx - margin), z2), new Point(tmpx, y3(tmpx), z2))
					.setEmission(new Color(GRAY)));
			tmpx -= widthx - margin;
		}
		...
		scene.lights.add(
				new PointLight(new Color(255, 153, 0), new Point(-1000, -1000, -200)).setKl(0.00001).setKq(0.0000001));
		scene.lights.add(new PointLight(new Color(YELLOW), new Point(-1000, -1000, -200)).setKl(0.0001).setKq(0.00001));
		scene.lights
				.add(new PointLight(new Color(MAGENTA), new Point(-1000, -1000, -200)).setKl(0.0001).setKq(0.000001));
		scene.lights
				.add(new PointLight(new Color(GREEN), new Point(-1000, -1000, -200)).setKl(0.0001).setKq(0.0000001));

		// scene.lights.add(new PointLight(new Color(RED), new Point(-102, 101,
		// -820)).setKl(0.0001).setKq(0.000001));
	...
	/**
	 * test for cbr only - manual bvh
	 */
	@Test
	void MiniProject2ManualTest() {
		Color grayColor = new Color(150, 150, 150);
		...
		while (tmpy < y1) {
			...
			g1.add(new Polygon(new Point(299, tmpy, z1), new Point(299, tmpy + margin, z1),
					new Point(299, tmpy + margin, z2), new Point(299, tmpy, z2)).setEmission(new Color(BLACK))
					.setMaterial(material1));
			tmpy += win_h + margin;// )
		}
		...
		while (tmpz < z2) {
			...
			g2.add(new Polygon(new Point(299, y2, tmpz), new Point(299, y2, tmpz + margin),
					new Point(299, y1, tmpz + margin), new Point(299, y1, tmpz)).setEmission(new Color(BLACK))
					.setMaterial(material1));
			tmpz += win_w + margin;
		}
		...
		Geometries totalBuilding1 = new Geometries(building1, g1, g2);
		...
		while (tmpy < y1) {
			...
			g3.add(new Polygon(new Point(x + margin, tmpy, z1), new Point(x + margin, tmpy + margin, z1),
					new Point(x + margin, tmpy + margin, z2), new Point(x + margin, tmpy, z2))
					.setEmission(new Color(GRAY)).setMaterial(material1));
			tmpy += win_h + margin;
		}
		...
		while (tmpz < z2) {
			...
			g4.add(new Polygon(new Point(x + 1, y2, tmpz), new Point(x + 1, y2, tmpz + margin),
					new Point(x + 1, y1, tmpz + margin), new Point(x + 1, y1, tmpz)).setEmission(new Color(GRAY))
					.setMaterial(material1));
			tmpz += win_w + margin;
		}
		...
		while (tmpx < x) {
			...
			g5.add(new Polygon(new Point(tmpx, y2 - 2, z1), new Point(tmpx - margin, y2 - 2, z1),
					new Point(tmpx - margin, y2 - 2, z2), new Point(tmpx, y2 - 2, z2)).setEmission(new Color(GRAY))
					.setMaterial(material1));
			tmpx += widthx + 1;
		}
		...
		while (tmpz < z2) {
			...
			g6.add(new Polygon(new Point(x, y2 - 2, tmpz), new Point(x, y2 - 2, tmpz + margin),
					new Point(-1060, y2 - 2, tmpz + margin), new Point(-1060, y2 - 2, tmpz))
					.setEmission(new Color(GRAY)).setMaterial(material1));
			tmpz += win_w + margin;
		}
		Geometries totalBuilding2 = new Geometries(building2, g3, g4, g5, g6); // right, up
		...
		tmpz = z1;
		margin = 1;
		Geometries g7 = new Geometries();

		while (tmpy < y1) {

			g7.add(new Polygon(new Point(x - 1, tmpy, z1), new Point(x - 1, tmpy + margin, z1),
					new Point(x - 1, tmpy + margin, z2), new Point(x - 1, tmpy, z2)).setEmission(new Color(GRAY)));
			tmpy += win_h + 1;
		}

		Geometries g8 = new Geometries();

		margin = 1.5;
		while (tmpz < z2) {

			g8.add(new Polygon(new Point(x - 1, y2, tmpz), new Point(x - 1, y2, tmpz + margin),
					new Point(x - 1, y1, tmpz + margin), new Point(x - 1, y1, tmpz)).setEmission(new Color(GRAY)));
			tmpz += win_w + margin;
		}
		...
		widthx = (x + 700) / 30 - 2 * margin;
		tmpz = z1;
		Geometries g9 = new Geometries();

		while (tmpx > x) {

			g9.add(new Polygon(new Point(tmpx, y2 - 2, z1), new Point(tmpx + margin, y2 - 2, z1),
					new Point(tmpx + margin, y2 - 2, z2), new Point(tmpx, y2 - 2, z2)).setEmission(new Color(GRAY)));
			tmpx -= widthx + margin;
		}

		Geometries g10 = new Geometries();

		margin = 1.5;
		while (tmpz < z2) {

			g10.add(new Polygon(new Point(x, y2 - 2, tmpz), new Point(x, y2 - 2, tmpz + margin),
					new Point(700, y2 - 2, tmpz + margin), new Point(700, y2 - 2, tmpz)).setEmission(new Color(GRAY)));
			tmpz += win_w + margin;
		}
		Geometries totalBuilding3 = new Geometries(building3, g7, g8, g9, g10);
		...
		tmpz = z1;
		widthx = (x + 200) / 10 - 2;
		Geometries g11 = new Geometries();

		while (tmpx > -200) {

			g11.add(new Polygon(new Point(tmpx, y1(tmpx), z1), new Point(tmpx - 1, y1(tmpx - 1), z1),
					new Point(tmpx - 1, y1(tmpx - 1), z2), new Point(tmpx, y1(tmpx), z2)).setEmission(new Color(GRAY)));
			tmpx -= widthx - 1;
		}
		margin = 1;
		tmpx = x;
		Geometries g12 = new Geometries();

		while (tmpx < 0) {

			g12.add(new Polygon(new Point(tmpx, y2(tmpx), z1), new Point(tmpx - margin, y2(tmpx - margin), z1),
					new Point(tmpx - margin, y2(tmpx - margin), z2), new Point(tmpx, y2(tmpx), z2))
					.setEmission(new Color(GRAY)));
			tmpx += widthx + 1;
		}
		...
		tmpx = -200;
		Geometries g13 = new Geometries();

		while (tmpx > -270) {

			g13.add(new Polygon(new Point(tmpx, y3(tmpx), z1), new Point(tmpx - margin, y3(tmpx - margin), z1),
					new Point(tmpx - margin, y3(tmpx - margin), z2), new Point(tmpx, y3(tmpx), z2))
					.setEmission(new Color(GRAY)));
			tmpx -= widthx - margin;
		}
		...
		Geometries totalBuilding4 = new Geometries(building4, g11, g12, g13);

		Geometries leftBuildings = new Geometries(totalBuilding2, totalBuilding4);
		Geometries rightBuildings = new Geometries(totalBuilding1, totalBuilding3);

		z1 = -6000;
		scene.geometries.add(leftBuildings, rightBuildings, new Polygon(new Point(z1, z1, z1), //
				new Point(z1, -z1, z1), new Point(-z1, -z1, z1), new Point(-z1, z1, z1))//
				.setEmission(new Color(150, 190, 230))//
				.setMaterial(new Material().setKd(0.8).setKs(0.1)));
		...
		new Camera(new Point(0, 0, 1000), new Vector(0, 0, -1), new Vector(0, 1, 0)) //
				.setVPSize(1000, 1500) //
				.setVPDistance(1000) //
				.setImageWriter(new ImageWriter("miniProject2Manual", 5000, 5000)) //
				.setRayTracer(new RayTracerBasic(scene)) //
				.setDebugPrint(0.2) //
				.setMultiThreading(3) //
				.renderImage() //
